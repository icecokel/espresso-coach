...
- 오입력 정정 삭제는 active 세션의 최신 `shotNumber`에만 허용한다.
- 이전 샷과 archived 세션의 샷은 삭제하지 않는다. 삭제 조건은 UI 표시와 별개로 repository transaction 안에서 다시 검증한다.
- 최신 샷 삭제 뒤 다음 저장은 남은 최신 번호 다음 값을 사용한다.
