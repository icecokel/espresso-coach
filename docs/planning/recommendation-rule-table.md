...
| --- | --- | --- | --- | --- |
| `R-UNDER-FAST` | `sour` tag + `brewTimeBand = short` | 분쇄도를 한 단계 곱게 | 추출 시간 확인, 추출량 소폭 증가 | 신맛과 짧은 시간은 과소추출 가능성을 높인다. |
| `R-UNDER-NORMAL` | `sour` tag + `brewTimeBand = normal or long` | 채널링 확인 | 레벨링/분배 확인, 분쇄도 조정 보류 | 시간이 충분한데 시면 균일 추출 문제가 섞였을 수 있다. |
| `R-OVER-SLOW` | `bitter` tag + `brewTimeBand = long` | 분쇄도를 한 단계 굵게 | 추출량 감소, 추출 시간 확인 | 쓴맛과 긴 시간은 과다추출 가능성을 높인다. |
...
| `R-WEAK-FAST` | `watery` or `hollow` + `brewTimeBand = short` | 분쇄도를 한 단계 곱게 | 추출 시간 확인 | 충분히 추출되지 않아 연하게 느껴질 수 있다. |
...
| `R-BALANCED` | `balanced` tag and no negative tag | 현재 설정 유지 | 기록 저장 | 맛이 만족스럽다면 변수를 바꾸지 않는다. |
...
| `C-NO-CHANGE-SWING` | 입력된 변경 없음 | 맛 태그 polarity가 크게 바뀜 | 탬핑/분배/채널링 확인 |
